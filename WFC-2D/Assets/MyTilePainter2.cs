...
public class MyTilePainter2 : MonoBehaviour
{
	...
	public void Drag(Vector3 mouse, MyTileLayorEditor2.TileOperation op)
	{
		Resize();
		if (tileobs == null) { Restore(); }
		if (this.ValidCoords((int)cursor.x, (int)cursor.y))
		{
			if (op == MyTileLayorEditor2.TileOperation.Sampling)
			{
			...
			else
			{
				...
				if (op == MyTileLayorEditor2.TileOperation.Drawing)
				{
...
#if UNITY_EDITOR
[CustomEditor(typeof(MyTilePainter2))]
public class MyTileLayorEditor2 : Editor
{
	...
	{
		MyTilePainter2 me = (MyTilePainter2)target;
	...
	private bool AmHovering(Event e)
	{
		...
		if (Physics.Raycast(HandleUtility.GUIPointToWorldRay(Event.current.mousePosition), out hit, Mathf.Infinity) &&
				hit.collider.GetComponentInParent<MyTilePainter2>() == me)
		{
